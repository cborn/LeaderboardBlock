...
//             LIMIT " . $limit;
    ...
    $sql = "SELECT $userfields,
        ...
        LEFT JOIN {badge} b ON b.id = bi.badgeid AND b.courseid = :courseid
        INNER JOIN {role_assignments} a ON a.userid = u.id AND a.roleid = :roleid
        INNER JOIN {context} c ON c.id = a.contextid AND c.id = :contextid
        GROUP BY u.id
...
//             LIMIT " . $limit;
    ...
    $sql = "SELECT $userfields,
        ...
        INNER JOIN {role_assignments} a ON a.userid = u.id AND a.roleid = :roleid
        INNER JOIN {context} c ON c.id = a.contextid AND c.id = :contextid
        WHERE bi.dateissued BETWEEN :weekstart AND :weekend
        GROUP BY u.id
        ORDER BY points DESC
        LIMIT " . $limit;
...
//     return $DB->get_record_sql($sql, $params);
    ...
    $badges = badges_get_user_badges($userid, $COURSE->id, 1, 6);
    return sizeof($badges);
}
